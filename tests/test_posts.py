...
def test_create_post_without_login(client):
    response = client.post("/posts/", json={"title": "No Login", "content": "Test"})
...
def test_update_post_correct_owner(authorized_client, db_session, test_user):
    ...
    response = authorized_client.patch(f"/posts/{post.id}", json={"title": "New Title"})
    assert response.status_code == 200
...
def test_update_post_wrong_owner(other_authorized_client, db_session, test_user):
    ...
    response = other_authorized_client.patch(f"/posts/{post.id}", json={"title": "New Title"})
...
def test_delete_post_correct_owner(authorized_client, db_session, test_user):
    ...
    # Validasi 404
    get_response = authorized_client.get(f"/posts/{post.id}")

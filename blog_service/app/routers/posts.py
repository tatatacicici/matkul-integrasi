...
from app.services.user_client import get_users_batch

router = APIRouter(prefix="/posts", tags=["Posts"])


def build_post_out(post: Post, author: AuthorOut | None = None) -> PostOut:
    """Map ORM → PostOut. Author di-inject dari hasil Data Stitching."""
    return PostOut(
        id=post.id,
        title=post.title,
        status=post.status,
...
def build_comment_out(c, author: AuthorOut | None = None) -> CommentOut:
    return CommentOut(
        id=c.id,
        comment=c.comment,
        post_id=c.post_id,
...
def get_all_posts(
    ...
    # 1. Kumpulkan semua user_id unik dari halaman ini
    user_ids = [p.user_id for p in posts]

    # 2. Batch call ke Auth Service — 1 request untuk semua user
    authors_map = get_users_batch(user_ids)

    # 3. Data Stitching: gabungkan Post dengan Author-nya
    return PaginatedResponse(
        data=[
            build_post_out(p, author=authors_map.get(p.user_id))
            for p in posts
        ],
        meta=PaginationMeta(
            total=total,
...
def get_post(post_id: int, db: Session = Depends(get_db)):
    ...
    # Kumpulkan semua user_id dari post + komentar-komentarnya
    all_user_ids = [post.user_id] + [c.user_id for c in post.comments]
    authors_map = get_users_batch(all_user_ids)
    ...
    return PostOutWithComments(
        ...
        link=f"/api/v1/posts/{post.id}",
        comments=[
            build_comment_out(c, author=authors_map.get(c.user_id))
...
def create_post(
    payload: PostCreate,
    db: Session = Depends(get_db),
    current_user: CurrentUser = Depends(get_current_user),  # auth middleware
):
    """
